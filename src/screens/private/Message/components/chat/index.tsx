...
import { chatDatabase, collegatoChatId, chatListRecadosEndPoint, messagesRecadosEndPoint, usuariosRecadosEndPoint } from '../../../../../services/firebaseConfigs/firebaseConfig';
...
const ChatOmbudsman: React.FC = () => {
    ...
    function buscarDadosUsuarioDestino() {
        ...
        const referenceChatList = ref(chatDatabase, `${chatListRecadosEndPoint}/${usuarioLogadoId}/${params.usuarioId}`);
        onValue(referenceChatList, (snapshot) => {
            tratarObjetoChatConfig(snapshot);
        });

        const referenceUsuarios = ref(chatDatabase, `${usuariosRecadosEndPoint}/${params.usuarioId}`);
    ...
    function sendMessage(): void {
        ...
        const referenceChatListOne = ref(chatDatabase, `${messagesRecadosEndPoint}/${params.salaId}`);
        push(referenceChatListOne, mensagemData).then(() => {
            ...
            const referenceChatListOne = ref(chatDatabase, `${chatListRecadosEndPoint}/${params.usuarioId}/${usuarioLogadoId}`);
            update(referenceChatListOne, chatListUpdate).then(() => console.log("Data updated")).catch(() => console.log("Erro ao atualizar a lista"));

            chatListUpdate.viuUltimaMensagem = true;
            const referenceChatListTwo = ref(chatDatabase, `${chatListRecadosEndPoint}/${usuarioLogadoId}/${params.usuarioId}`);
            update(referenceChatListTwo, chatListUpdate).then(() => console.log("Data updated")).catch(() => console.log("Erro ao atualizar a lista do usuário logado"));
            setNewMessage("");
        });
