...
import { Avatar, Button, Divider, IconButton, List, Subheading, Dialog, Paragraph } from 'react-native-paper';
...
import { adicionarMascara, removerPontuacaoDocumento, validarCNPJ, validarCPF, validarData } from '../../../helpers/formHelpers';
...
const MyProfile: React.FC = () => {
    const { dataLoginUser, usuarioLogado, pessoaLogada, setPessoaLogada, setUsuarioLogado, limparLogin } = useContext<ILoginData>(AuthContext);
    ...
    const setDefaultValuesUser: Function = (pessoa: Pessoa): void => {
        ...
        if (pessoa.cep)
            setValue('cep', adicionarMascara(pessoa.cep, 'zip-code'));

        if (pessoa.numero)
            setValue('numero', pessoa.numero.toString());
        ...
        <>
            ...
            <View style={[styles.contain]}>
                    ...
                    <>
                        <View style={styles.usuarioFotoContainer}>
                            <View style={styles.photoInputAvatarButton}>
                                <Avatar.Image size={100} source={{ uri: `data:image/jpeg;base64,${pessoaLogada.foto}` }} />
                            </View>
                        ...
                        {usuarioEditando && pessoaEditando && (
                            <ScrollView style={styles.usuarioDadosContainer}>
                                <List.Item
                                    ...
                                    right={props => <Subheading>{usuarioEditando.login}</Subheading>}
                                />
                                <Divider />

                                <List.Item
                                    title="Nome"
                                    right={props => <Subheading>{pessoaEditando.nome}</Subheading>}
                                />
                                <Divider />

                                <List.Item
                                    title="Documento"
                                    right={props => <Subheading>{adicionarMascara(pessoaEditando.cpfCnpj, 'document')}</Subheading>}
                                />
                                <Divider />

                                <List.Item
                                    title="Telefone"
                                    right={props => <Subheading>{adicionarMascara(pessoaEditando.telefone, 'cel-phone')}</Subheading>}
                                />
                                <Divider />

                                <List.Item
                                    title="Município"
                                    right={props => <Subheading>{pessoaEditando.municipio}</Subheading>}
                                />
                                <Divider />

                                <List.Item
                                    title="Estado"
                                    right={props => <Subheading>{pessoaEditando.estado}</Subheading>}
                                />

                                <List.Item
                                    title="Endereço"
                                    right={props => <Subheading>{pessoaEditando.endereco}</Subheading>}
                                />

                                <List.Item
                                    title="Número"
                                    right={props => <Subheading>{pessoaEditando.numero}</Subheading>}
                                />

                                <List.Item
                                    title="País"
                                    right={props => <Subheading>{pessoaEditando.pais}</Subheading>}
                                />
                            </ScrollView>
                        )}
                    </>
                ...
                <Dialog visible={dialogVisible} onDismiss={() => setDialogVisible(false)}>
                    <Dialog.Title>Excluindo Conta - {usuarioEditando && usuarioEditando.pessoa.nome}</Dialog.Title>
                    ...
                    <Dialog.Content>
                        <Paragraph>Deseja realmente excluir sua conta? Todas os agendamentos e informações relacionados a sua conta serão excluidos!</Paragraph>
                    </Dialog.Content>
