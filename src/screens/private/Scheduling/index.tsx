...
import { View, Text, FlatList, RefreshControl, Modal as ModalNative, Linking } from 'react-native';
...
const Scheduling: React.FC = () => {
    ...
    const [salasEmpresaSelecionada, setSalasEmpresaSelecionada] = useState<Sala[]>([]);
    ...
    async function alterarPaginacao() {
        if (empresas.length >= paginacaoInicial.take) {
            ...
            let filtro: EmpresasFiltro = {
                ...novaPaginacao,
                categoriaId: _categoriaIdParam > 0 ? _categoriaIdParam : Number(categoriaId),
    ...
    async function buscarSalasDaEmpresa(empresaId: number): Promise<Sala[]> {
        ...
        let resultado = await getSalasPorEmpresaPaginado(empresaId, { order: 'DESC' });
        if (!resultado.success) {
    ...
    function onPressAgendar(salaId: number) {
        closeModalRoom();
        navigation.navigate(ROUTES.agendar, { empresaId: empresaSelecionada.id, salaId: salaId });
    }
                ...
                renderItem={({ item }) => (
                    <>
                        <Card
                            ...
                            left={<Avatar.Image size={60} source={{ uri: `data:image/jpeg;base64, ${item.logo}` }} />}
                        >
                            <>
                                <Divider />
                                <View>
                    ...
                    headerContent={(
                        <View style={styles.modalRoomHeader}>
                            <View style={styles.modalRoomPhoto}>
                                <Avatar.Image size={60} source={{ uri: `data:image/jpeg;base64,${empresaSelecionada.logo}` }} />
                            </View>
                ...
                >
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <Card
                                title={item.nome}
                                subtitleStyle={{minHeight: 20}}
                                subtitle={item.multiplasMarcacoes ? "Você pode agendar mais de um horário" : ""}
                                right={(
                                    <Button
                                        mode="contained"
