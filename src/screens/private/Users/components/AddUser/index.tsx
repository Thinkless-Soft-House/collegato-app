...
let usuarioSchema = yup.object({
    ...
    senhaConfirma: yup.string().oneOf([yup.ref('senha'), null], 'As senhas devem ser iguais'),
...
const AddUser: React.FC = () => {
    ...
    async function buscarDadosIniciais() {
        try {
            ...
            if (usuarioLogado.permissaoId != PermissaoEnum.Administrador) {
                setPermissoes(permissoes.filter(e => e.value == PermissaoEnum.Funcionario));
            }
    ...
    function setDefaultValues(pessoa: Pessoa, usuario: Usuario): void {
        ...
        if (usuario.empresaId)
            setEmpresaId(usuario.empresaId.toString());
    };
    ...
    function usuarioPrecisaPossuirEmpresa(permissaoId: number): boolean {
        return permissaoId == PermissaoEnum.Empresario || permissaoId == PermissaoEnum.Funcionario;
    }
    ...
    const submitForm = async (form: FormularioObjeto): Promise<void> => {
        ...
        if (form.permissaoId != PermissaoEnum.Funcionario && form.permissaoId != PermissaoEnum.Empresario) {
            _empresaId = 0;
        }
    ...
    return (
        <View style={[styles.contain]}>
            ...
            <ScrollView style={[styles.mtContent]}>
                ...
                {usuarioPrecisaPossuirEmpresa(permissaoId) && usuarioLogado.permissaoId != PermissaoEnum.Empresario && (
                    <SearchDropDown
