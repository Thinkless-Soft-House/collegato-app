import { View, FlatList, Modal as ModalNative, RefreshControl } from 'react-native';
import React, { useCallback, useContext, useEffect, useState, useRef } from 'react';
import { Button, MD2Colors, Dialog, IconButton, Modal, Paragraph, Searchbar, Title } from 'react-native-paper';
...
const Users: React.FC = () => {
    ...
    async function buscarPermissoes() {
        ...
        if (usuarioLogado.permissaoId != PermissaoEnum.Administrador)
            novaLista = novaLista.filter(e => e.value != PermissaoEnum.Administrador.toString());
    ...
    return (
        <View style={[styles.contain]}>
            <SemiHeader goBack={onPressBack}>
                <Button mode="contained" onPress={() => navigation.navigate(ROUTES.adicionarUsuario)}>
                    Adicionar
                </Button>
            </SemiHeader>
            ...
            <Dialog visible={dialogVisible} onDismiss={() => setDialogVisible(false)}>
                <Dialog.Title>Excluindo o usuário - {usuarioDelete && usuarioDelete.pessoa.nome}</Dialog.Title>
                {showProgressBar && (
                    <MotiView
                        style={[styles.containProgressBar]}
                        from={{
